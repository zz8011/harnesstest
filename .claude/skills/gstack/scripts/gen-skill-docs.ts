...
function generateDocVoice(_ctx: TemplateContext): string {
  return `Use the gstack doc voice guide:

- Lead with what the user can now **do** or why the change matters
- Use plain language, not implementation details
- Prefer phrasing like "You can now..." over "Refactored..." or "Added support for..."
- Keep internal or contributor-only details separate from user-facing bullets
- Make each bullet feel useful and concrete, not like a commit message`;
}

function generateQaCiGate(ctx: TemplateContext): string {
  return `## Optional: CI/CD QA Gate

If the user asks to wire \`/qa\` into GitHub Actions, treat it as an explicit setup task.
Only do this when all of the following are true:

- The repo uses GitHub Actions
- There is a stable QA target URL (preview deploy or staging URL)
- Claude auth can be provided in CI secrets or vars:
  - direct Anthropic: \`ANTHROPIC_API_KEY\`
  - Anthropic-compatible gateway: \`ANTHROPIC_AUTH_TOKEN\` + \`ANTHROPIC_BASE_URL\`
- gstack is vendored into the repo, or the workflow can fetch the same version in CI

Recommended minimum path:

1. Copy the reference workflow from \`${ctx.paths.skillRoot}/qa/templates/github-qa-gate.yml\` into \`.github/workflows/qa.yml\`
...
- Missing baseline on first adoption is **non-blocking**; use \`--allow-missing-baseline\`
- Fail the workflow only when health score drops beyond the configured threshold
- Do **not** auto-update the committed baseline in the same PR gate run
- If the repo uses an Anthropic-compatible gateway, also mirror any required model selection vars (for example \`ANTHROPIC_MODEL\`) into GitHub repo variables
- If the repo does not use GitHub Actions, skip this setup and explain that the current template only covers GitHub Actions`;
}
